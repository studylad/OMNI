...
def data_read(args):
    ...
    for i in range(0,len(file_dirs),7):
        if (i == 7 * patient_no):
            ...
            print(f'-------- Acquiring data from patient {patient_no + 1} --------')
...
def windowing_and_resampling_hr(patient):
    ...
    windowed_patient = {
        'ecg': [[] for _ in range(10)],
        'resp': [[] for _ in range(10)],
    }

    windowed_patient_overlap = {
        'ecg': [[] for _ in range(10)],
        'resp': [[] for _ in range(10)],
    }

    for infant_no, patients in enumerate(patient[ 'ecg']):
    ...
    for infant_no, patients in enumerate(patient['resp']):

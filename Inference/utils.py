...
def obtain_ecg_record(path_records,fs,window_size = 5000):
    ...
    for path in path_records:
        ...
        actual_ecg_windows.extend(
            scaler.fit_transform(
                ECG[5000 * record_no : 5000 * record_no + window_size].reshape(
                    -1, 1
                )
            )
...
def load_model_HR(SAVED_MODEL_PATH,test_loader,device,batch_len,window_size):
    ...
    with torch.no_grad():
        ...
        for pred_peak_ in pred_peaks:
            peak_array = np.hstack((peak_array, pred_peak_))
...
def load_model_BR(SAVED_MODEL_PATH,test_loader,device,batch_len,window_size):
    ...
    for x in test_loader:
        ...
        breathsPosition = (
            np.append(breathsPosition, valleys[valleys < middle_end])
            if start == 0
            else np.append(breathsPosition, current + startBR)
        )
...
def findValleys(signal, prominence = 10, is_smooth = True , distance = 10):
    ...
    return scipy.signal.find_peaks(smoothened, prominence=0.05,  distance = 125)[0]

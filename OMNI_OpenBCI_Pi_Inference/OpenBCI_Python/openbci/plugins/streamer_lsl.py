...
class StreamerLSL(plugintypes.IPluginExtended):
    # From IPlugin
    def activate(self):
        eeg_stream = self.args[0] if len(self.args) > 0 else "OpenBCI_EEG"
        eeg_id = self.args[1] if len(self.args) > 1 else "openbci_eeg_id1"
        aux_stream = self.args[2] if len(self.args) > 2 else "OpenBCI_AUX"
        aux_id = self.args[3] if len(self.args) > 3 else "openbci_aux_id1"
        imp_stream = self.args[4] if len(self.args) > 4 else "OpenBCI_Impedance"
        imp_id = self.args[5] if len(self.args) > 5 else "openbci_imp_id1"
        # Create a new streams info, one for EEG values, one for AUX (eg, accelerometer) values
        print(
            (
                (
                    (
                        f"Creating LSL stream for EEG. Name:{eeg_stream}- ID:{eeg_id}"
                        + "- data type: float32."
                    )
                    + str(self.eeg_channels)
                    + "channels at"
                )
        ...
        print(
            (
                (
                    (
                        f"Creating LSL stream for AUX. Name:{aux_stream}- ID:{aux_id}"
                        + "- data type: float32."
                    )
                    + str(self.aux_channels)
                    + "channels at"
                )
                + str(self.sample_rate)
                + "Hz."
            ...
            print(
                (
                    (
                        (
                            (
                                f"Creating LSL stream for Impedance. Name:{imp_stream}- ID:"
                                + imp_id
                            )
                            + "- data type: float32."
                        )
                        + str(self.imp_channels)
                        + "channels at"
                    )
                    + str(self.sample_rate)
                    + "Hz."
